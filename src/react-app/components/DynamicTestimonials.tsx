  ...
  const fetchTestimonials = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (showOnlyFeatured) {
          filteredTestimonials = filteredTestimonials.filter((t: Testimonial) => t.is_featured);
        }
  ...
  return (
    <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 ${className}`}>
      {testimonials.map((testimonial) => (
        <div
          key={testimonial.id}
          className="bg-white rounded-lg shadow-lg p-6 border-l-4 border-blue-500 hover:shadow-xl transition-shadow duration-300"
        >
