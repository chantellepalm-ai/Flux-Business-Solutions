...
export default function Packages() {
    ...
    return (<section className="min-h-screen bg-black py-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-16">
          {packages.map((pkg, index) => {
            const Icon = pkg.icon;
            return (<div key={index} className={`relative bg-black rounded-2xl p-8 border-2 border-${pkg.borderColor}/50 hover:border-${pkg.borderColor} transition-all duration-300 hover:shadow-${pkg.borderColor} hover:scale-105 ${pkg.popular ? `ring-2 ring-${pkg.borderColor} ring-offset-2 ring-offset-black shadow-${pkg.borderColor}` : ''}`}>
                ...
                {/* CTA Button */}
                <Link to="/contact" className={`block w-full text-center bg-gradient-to-r ${pkg.gradient} text-black font-bold py-3 px-6 rounded-lg hover:opacity-90 transition-all duration-300 shadow-${pkg.borderColor} hover:scale-105 border-2 border-${pkg.borderColor}/50 hover:border-${pkg.borderColor}`}>
                  Get Started
                </Link>
              </div>);
        })}
        ...
        <div className="text-center bg-black border-2 border-neon-purple/50 rounded-2xl p-12 shadow-neon-purple">
          ...
          <Link to="/contact" className="inline-block bg-gradient-to-r from-neon-pink to-neon-purple text-white font-bold py-4 px-8 rounded-full shadow-neon-multi border-2 border-neon-pink/50 hover:border-neon-pink transition-all duration-300 hover:scale-105">
