...
app.post("/api/contact", zValidator("json", contactSchema), async (c) => {
  try {
    ...
    const result = await c.env.DB.prepare(
      "INSERT INTO contact_leads (name, email, phone, company, message) VALUES (?, ?, ?, ?, ?)"
    ).bind(data.name, data.email, data.phone || null, data.company || null, data.message).run();
...
app.post("/api/appointments", zValidator("json", appointmentSchema), async (c) => {
  try {
    ...
    const result = await c.env.DB.prepare(
      "INSERT INTO appointments (name, email, phone, company, service_type, preferred_date, preferred_time, message) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    ).bind(
      data.name, 
      data.email, 
      data.phone || null, 
...
app.post("/api/sync-rss-feed", async (c) => {
  try {
    ...
    for (const item of feed.items) {
      ...
      const existing = await c.env.DB.prepare(
        "SELECT id FROM external_blog_posts WHERE link = ?"
      ).bind(item.link).first();
      
      if (existing) {
        // Update existing post
        await c.env.DB.prepare(
          "UPDATE external_blog_posts SET title = ?, excerpt = ?, image_url = ?, published_date = ?, author = ?, category = ?, updated_at = CURRENT_TIMESTAMP WHERE link = ?"
        ).bind(
          item.title,
          excerpt,
          imageUrl,
          item.isoDate || new Date().toISOString(),
          item.creator || item.author || 'Marketing Dive',
          category,
          item.link
        ).run();
        updatedCount++;
        ...
        // Insert new post
        await c.env.DB.prepare(
          "INSERT INTO external_blog_posts (external_id, title, link, excerpt, image_url, published_date, author, category) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
        ).bind(
          item.guid || item.link,
          item.title,
          item.link,
...
app.get("/api/testimonials", async (c) => {
  try {
    const testimonials = await c.env.DB.prepare(
      "SELECT * FROM testimonials WHERE is_active = true ORDER BY is_featured DESC, created_at DESC"
    ).all();
...
app.post("/api/testimonials", zValidator("json", testimonialSchema), async (c) => {
  try {
    ...
    const result = await c.env.DB.prepare(
      "INSERT INTO testimonials (client_name, client_company, client_position, testimonial_text, rating, is_featured) VALUES (?, ?, ?, ?, ?, ?)"
    ).bind(
      data.client_name,
